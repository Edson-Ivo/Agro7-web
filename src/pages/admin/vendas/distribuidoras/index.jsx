...
function AdminVendasDistribuidoras() {
  ...
  const { data, error, mutate } = useFetch(
    `/distributors/find/all?limit=${perPage}&page=${page}&search=${search}`
  );
  ...
  return (
    <>
      ...
      <Container>
        ...
        <Section>
          <SectionHeader>
            <SectionHeaderContent title="Gerenciar Distribuidoras" />
          </SectionHeader>
          <SectionBody>
            <div className="SectionBody__content">
              <CardContainer>
                ...
                <InputSearch
                  url="/admin/vendas/distribuidoras"
                          ...
                          {(!isEmpty(data?.items) &&
                            data.items.map(p => (
                              <tr
                                ...
                                onClick={() =>
                                  router.push(
                                    `/admin/vendas/distribuidoras/${p?.id}/detalhes`
                                  )
                                }
                              >
                                <td>{p?.name}</td>
                                ...
                                <td onClick={e => e.stopPropagation()}>
                                  <ActionButton
                                    id={p.id}
                                    path={`/admin/vendas/distribuidoras/${p?.id}`}
                                    noEdit
...
export default privateRoute([usersTypes[0]])(AdminVendasDistribuidoras);
