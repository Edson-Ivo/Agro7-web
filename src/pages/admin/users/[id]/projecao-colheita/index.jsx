...
function UserProjecaoColheita() {
  ...
  const { data: dataChart, error: errorChart } = useFetch(
    dateStart && dateEnd
      ? `/charts/find/harvests-projection/by/user/${id}?date_start=${dateStart}&date_finish=${dateEnd}`
      : null
  );

  const { data: dataChartProduct, errorChartProduct } = useFetch(
    dateStart && dateEnd && selectedProduct
      ? `/charts/find/harvests-date/by/product/${selectedProduct}/user/${id}?date_start=${dateStart}&date_finish=${dateEnd}&is_green=${dataSelectedProductIsGreen}`
      : null
  );
  ...
  if (errorChart || errorChartProduct || errorUser)
    return <Error error={errorChart || errorChartProduct || errorUser} />;
          ...
          <SectionBody>
            <div className="SectionBody__content">
              <CardContainer>
                ...
                <InputDateInterval
                  url={`/admin/users/${id}/projecao-colheita`}
                  onError={setAlertMsg}
                        ...
                        !isEmpty(dataProductTotal) ? (
                          <>
                            <Alert type="info">
                              ...
                              produto para ver a projeção de colheitas
                            </Alert>
