...
function VendasEtiquetasCaixas() {
  ...
  const { data: dataLabel, error: errorLabel } = useFetch(
    dataSale && quantity && validate
      ? `/sales/generate/box-label/by/id/${dataSale?.id}?quantity=${quantity}&date_validate=${validate}`
      : null,
    true
  );
                      ...
                      <>
                        {(!isEmpty(dataLabel) && (
                          <PDFViewer
                            ...
                            pdfName={`Venda ${dataSale?.batch} - Etiqueta para Caixas ou Sacarias.pdf`}
...
export default privateRoute()(VendasEtiquetasCaixas);
